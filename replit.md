...
- **Messages:** Features for sending messages, an Inbox with a three-panel conversation workspace (conversation list, chat pane, contact info sidebar), and Campaign History. The Inbox uses a hybrid flex/absolute positioning layout and robust JavaScript for filtering and interaction.
  - **Inbox Rich RCS Integration:** Rich RCS messages created in the wizard render directly in the chat thread after sending. Uses the shared `RcsPreviewRenderer` for consistent visual output between wizard preview and thread display. Includes: `sendRichRcsFromWizard()` (handles send flow with API call structure marked as TODO), `renderRichRcsInThread()` (appends card/carousel to chat), `mapPayloadCardToRendererFormat()` (maps payload to renderer format with height value normalization), `updateConversationSnippet()` (updates conversation list), `resetRcsComposer()` (clears wizard state after send). Fallback renderer `renderRichCardHtmlInbox()` handles cases when shared renderer unavailable.
...
- **Unified RCS Wizard:** The RCS Rich Card Wizard is a shared fullscreen modal component (`resources/views/quicksms/partials/rcs-wizard-modal.blade.php`) used identically across Send Message and Inbox pages via `@include`. Both pages load the shared `public/js/rcs-wizard.js` for wizard functionality. JavaScript deduplication completed (Jan 2026): Send Message removed ~1400 lines of duplicate inline JS, now relying on shared rcs-wizard.js for all core wizard functions. Page-specific helpers retained: `getRcsPayloadForSubmission`, `processAssetServerSide`, `setRcsCropPosition`, `insertRcsPlaceholder`, `insertRcsEmoji`, `captureRcsMessageTypeState`, `escapeHtml`.
- **RCS Preview System:** A schema-driven renderer at `/rcs/preview-demo` providing an Android-style phone UI for previewing RCS messages, built with Alpine.js. The shared renderer (`public/js/rcs-preview-renderer.js`) now supports channel-specific rendering:
  ...
  - **Mock RCS Agents:** Two mock agents with 222x222 SVG brand icons stored locally at `/public/images/rcs-agents/`. "QuickSMS Brand" (purple) and "Promotions Agent" (pink) auto-select when switching to RCS channels. Agent data shape matches future RCS Registration page (id, name, logo, tagline, brand_color).
- **RCS Asset Management:** Server-side image processing pipeline for RCS media. When users provide public image URLs and apply edits (zoom, crop, orientation), the system fetches images server-side, applies transformations using Intervention Image, and stores on QuickSMS-hosted storage. Supports files up to 100MB. Features:
  ...
  - **Save-Time Preview Sync:** Phone preview updates ONLY when user clicks Save - no live preview during drag/zoom. The saved/hosted image is the single source of truth for wizard preview, Send Message preview, and final RCS payload. After save, previews display the hosted URL directly at native size (no CSS transforms or recalculation). Backend uses exact crop parameters from editor (cropOffsetX/Y, frameWidth/Height, displayScale) to produce pixel-perfect output matching the crop frame.
