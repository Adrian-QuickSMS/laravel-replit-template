...
- **Bootstrap Grid Layout:** Uses standard three-column layout (col-xl-3/6/3) matching other Fillow screens with separate cards for each panel.
- **Independent Scrolling:** Left panel uses `height: calc(80vh - 200px)` with `overflow-y: auto`. Chat pane uses flex column layout with `height: calc(80vh - 100px)`, chat-box-area has `flex: 1; overflow-y: auto; min-height: 0`, and composer stays fixed at bottom with `flex-shrink: 0`. Right sidebar uses `height: calc(80vh - 150px)` with `overflow-y: auto`.
...
- **Chat Pane (Center):** Uses `.message-received` (gray, left-aligned) and channel-specific `.message-sent` bubbles (green gradient for SMS, blue gradient for RCS) with timestamps and delivery indicators. Both bubble types use max-width: 70% and min-width: 100px for sensible width. Source display in header ("From: [VMN/Short Code/RCS Agent]")
