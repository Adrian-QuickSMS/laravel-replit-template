...
- **Admin Control Plane:** A separate internal interface for QuickSMS employees with a hard security boundary, separate authentication, mandatory MFA, IP allow-listing, and distinct RBAC. Features include:
  ...
  - **Admin Global Templates Library:** Cross-tenant template management at `/admin/management/templates` with Admin Blue theme. Features:
    ...
    - Row actions: View Details, Edit (non-archived), Suspend (live), Reactivate (paused), Archive (non-archived)
    ...
    - **Template Lifecycle Actions:** Suspend, Reactivate, Archive with confirmation modals matching customer portal patterns
      - Suspend: requires reason, changes status from live to paused
      - Reactivate: changes status from paused to live
      - Archive: requires reason, permanently archives template (cannot be used by customer)
    - **Admin Audit Logging (Admin-only):** All lifecycle actions emit admin audit events (not customer audit logs)
      - Events: TEMPLATE_VIEWED, TEMPLATE_EDITED, TEMPLATE_SUSPENDED, TEMPLATE_REACTIVATED, TEMPLATE_ARCHIVED
      - Each event includes: adminUserId, adminEmail, accountId (customer), templateId, templateName, before/after snapshot, reason (where applicable), sourceScreen, timestamp
      - IP address captured server-side via AdminControlPlane.logAudit
      - Before/after snapshots capture status changes without message content or PII
