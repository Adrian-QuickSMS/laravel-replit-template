...
- **Purchase:** Functionality to purchase messages (Admin/Finance only) with HubSpot Products API integration for live pricing and VAT calculation. Includes UI for product selection, order summary, and payment flow with Stripe (webhook handling for payment status).
  ...
  - **Top Up Balance Flow:** For prepaid accounts. Entry point is "Top Up Balance" button in Account Financial Summary. Full Purchase-style modal with tier cards (Starter/Enterprise) matching the Purchase Messages page design: gradient backgrounds, noUiSlider volume sliders, numeric inputs for volume/cost, and Order Summary sidebar with cost breakdown (Net Total, VAT 20%, Total Payable). Bespoke pricing accounts see a dedicated Custom Contract tier with 50K-5M volume range. Single-step flow: select tier, adjust volume via slider, then Proceed to Payment redirects to Stripe. Tier volume ranges: Starter (0-50K, 1K increment), Enterprise (50K-1M, 50K increment), Bespoke (50K-5M, 50K increment). Minimum validation: 1K for Starter, 50K for Enterprise/Bespoke.
