...
- **Inbox / Conversation Workspace:** A three-panel interface at `/messages/inbox` with a conversation list (searchable, filterable, sortable, unread badges), a chat pane (displaying sent/received messages with channel-specific styling), and a collapsible contact info sidebar. Features a robust JavaScript-based filter system, rich conversation item layouts, and a reply composer with channel picker, template support, and AI assistance. Includes inline rendering for RCS rich cards. **Layout Solution:** Uses CSS Grid (`grid-template-rows: auto auto 1fr auto`) for the chat pane to handle header, optional search bar, messages area, and composer. This avoids conflicts with Fillow's `.chat-box-area` fixed heights and dz-scroll plugin inline styles. Custom `.qs-chat-messages` class replaces Fillow's class to prevent CSS conflicts.
