...
- **API Connections:** `api_connections`, `api_connection_audit_events`. Suspension distinguishes admin vs customer via `suspended_by_type` column. Admin-suspended connections cannot be reactivated by customers (enforced server-side). Admin can escalate customer suspensions to admin-level. Customer portal shows pastel red badge for admin suspensions, pastel yellow for customer suspensions.
...
- **Flow Builder:** Visual drag-and-drop flow builder for automated messaging journeys. Tables: `flows` (RLS-protected, soft-deletes), `flow_nodes`, `flow_connections`, `api_credentials` (encrypted credential vault, RLS-protected, soft-deletes). Controllers: `FlowBuilderController`, `ApiCredentialController`. Routes at `/flows/*` and `/api-credentials`. 21 node types across 4 categories: triggers (API, webhook, SMS inbound, RCS inbound, campaign event, contact event, schedule), actions (send message, contact, tag, list, opt-out, webhook with dual success/error ports, action group with 8 step types), logic (wait, decision, contact decision, webhook decision with yes/no branching), endpoints (inbox handoff, flow handoff, end). Credential vault stores API keys/tokens encrypted at rest (`encrypted:array` cast, `$hidden`). Credential write routes gated by `permission:manage_api_credentials` (owner/admin/developer). All credential CRUD audited via `AccountAuditLog`. Node types validated server-side via `in:` whitelist; action group step types validated against 8 allowed values. Full reference doc: `FLOW_BUILDER_PHASE2.md`.
- **Admin Landing Page:** `/admin` is a standalone landing page (no sidebar) with two area cards: "Messaging Platform" → `/admin/messaging/dashboard`, and "HR Management" → `/admin/hr/dashboard`. HR card only visible if user has HR access. After login, all redirects go to this landing page. Old `/admin/dashboard` route redirects to `/admin/messaging/dashboard` for backwards compatibility.
- **Admin Layouts:** Two separate admin layouts: `layouts/admin.blade.php` (messaging, uses `admin-sidebar.blade.php`) and `layouts/admin-hr.blade.php` (HR, uses `admin-hr-sidebar.blade.php`). Both sidebars include an "Admin Home" link back to the landing page. HR section removed from the messaging sidebar.
- **HR Leave Management (Admin Console):** Internal staff leave management module at `/admin/hr/*`. Tables: `employee_hr_profiles`, `leave_entitlements`, `leave_requests`, `leave_audit_log`, `hr_settings`, `bank_holidays`, `holiday_adjustment_requests` (no `tenant_id` — admin-only data). `admin_users` extended with `hr_role` (none/employee/manager/hr_admin) and `birthday` columns. Default entitlement: 116 units (29 days). Leave types: annual, sickness, medical, birthday (auto-approved, validated against actual birthday). Bank holidays pre-seeded for England & Wales 2026-2027, with GOV.UK API import. Controller: `HrController`. Services: `LeaveCalculationService`, `LeaveRequestService`, `HolidayAdjustmentService`, `BradfordFactorService`, `HrNotificationService`. Auth: `AdminAuthenticate` middleware + `hasHrAccess()` / `isHrManager()` / `isHrAdmin()` role checks. Views: dashboard (with Bradford Factor scores), my-leave (with purchase holiday), team-calendar (FullCalendar.js with department filter), settings (employee management, pending approvals, adjustments, bank holiday CRUD, webhook config, carry-over). HR views use `layouts/admin-hr` layout with dedicated HR sidebar. Phase 2 features: purchased holiday (HR Admin approval), TOIL/gifted days (manager/admin grant), year-end carry-over (`hr:carry-over` artisan command with `--dry-run`), manager-scoped approvals (direct reports only, self-approval prevented), bank holiday CRUD + GOV.UK import, Bradford Factor scoring (S²×D, 5 threshold bands), Slack/Teams webhook notifications. Shared 5-day (20 unit) additional holiday cap across carried-over + purchased + gifted.
