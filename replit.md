...
*   **Campaign Opt-out System:** Full opt-out system implemented. Two mechanisms: reply-to-opt-out (keyword on VMN/shortcode) and click-to-opt-out (unique per-recipient URL via https://qout.uk/). Backend: `campaign_opt_out_urls` table (RLS, 30-day TTL), `OptOutService`, `CampaignOptOutUrl` model, `OptOutLandingController` (public routes `/o/{token}` + `/o/{token}/confirm`), 3 public landing views. API: 4 endpoints on `/api/campaigns/` (`opt-out-numbers`, `validate-opt-out-keyword`, `suggest-opt-out-text`, `opt-out-keywords/{numberId}`). UI: Send Message opt-out card fully wired — number selector (VMN + shortcodes, dynamic, grouped), keyword input (text for VMN/dedicated, select for shared shortcode), live validation, auto-suggested text, insert-to-message button, opt-out list selector. Opt-out fields persisted on Campaign model (`opt_out_enabled`, `opt_out_method`, `opt_out_number_id`, `opt_out_keyword`, `opt_out_text`, `opt_out_list_id`, `opt_out_url_enabled`).
