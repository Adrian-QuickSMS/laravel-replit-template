...
<div class="page-header">
    <div>
        <h2>UK Routes</h2>
        <p>Manual routing control by UK mobile network</p>
    </div>
...
function changeProduct(product) {
    window.location.href = '{{ route("admin.routing.uk-routes") }}?product=' + product + '&show_blocked={{ $showBlocked ? "1" : "0" }}';
}

function toggleBlocked() {
    const checked = document.getElementById('showBlocked').checked;
    window.location.href = '{{ route("admin.routing.uk-routes") }}?product={{ $productType }}&show_blocked=' + (checked ? '1' : '0');
}
