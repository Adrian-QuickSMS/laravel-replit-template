...
class AuditLogApiController extends Controller
{
    ...
    public function stats(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $actorSql = "SELECT COUNT(DISTINCT user_id) as cnt FROM ("
                . "SELECT user_id::TEXT AS user_id FROM campaign_audit_log WHERE account_id = ? AND created_at >= ? AND user_id IS NOT NULL"
                . " UNION ALL SELECT user_id::TEXT AS user_id FROM user_audit_log WHERE account_id = ? AND created_at >= ? AND user_id IS NOT NULL"
                . " UNION ALL SELECT user_id::TEXT AS user_id FROM account_audit_log WHERE account_id = ? AND created_at >= ? AND user_id IS NOT NULL"
                . " UNION ALL SELECT user_id::TEXT AS user_id FROM number_audit_log WHERE account_id = ? AND created_at >= ? AND user_id IS NOT NULL"
                . ") AS actors";
    ...
    private function buildSourceQuery(
        ...
        if (!empty($src['custom_select'])) {
            ...
            $select = "SELECT id::TEXT AS id, '{$module}' AS module, '{$category}' AS category, action, user_id::TEXT AS user_id, user_name, details, COALESCE(metadata, '{}')::TEXT AS metadata, ip_address::TEXT AS ip_address, created_at{$entityCol} FROM {$table}";
        }
