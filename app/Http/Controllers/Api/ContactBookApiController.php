...
class ContactBookApiController extends Controller
{
    ...
    public function bulkAddToOptOut(Request $request): JsonResponse
    {
        ...
        $list = OptOutList::find($request->input('opt_out_list_id'));
        if (!$list) {
        ...
        DB::transaction(function () use ($contacts, $list, $accountId, &$inserted) {
            foreach ($contacts as $contactId => $mobileNumber) {
                if (!$mobileNumber) continue;

                $exists = DB::table('opt_out_records')
                    ->where('opt_out_list_id', $list->id)
        ...
        return response()->json([
            'success' => true,
            'message' => "Added {$inserted} number(s) to opt-out list \"{$list->name}\"",
            'affectedCount' => $inserted,
        ]);
    }
    ...
    public function bulkRemoveFromOptOut(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => "Removed {$deleted} number(s) from opt-out list \"{$list->name}\"",
            'affectedCount' => $deleted,
        ]);
    }
