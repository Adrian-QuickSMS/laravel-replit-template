...
class UserManagementController extends Controller
{
    ...
    public function invite(Request $request)
    {
        try {
            ...
            try {
                $actor = AuditContext::actor();
                UserAuditLog::record($actor['account_id'], 'user_invited', null, $actor['user_id'], $actor['user_name'], "Invitation sent to {$invitation->email}", ['invitee_email' => $invitation->email, 'assigned_role' => $invitation->role, 'sub_account_id' => $invitation->sub_account_id ?? null, 'invitation_id' => $invitation->id]);
            } catch (\Throwable $e) {
                Log::warning('[AuditLog] Failed to record user_invited', ['error' => $e->getMessage()]);
            }
    ...
    public function revokeInvitation(Request $request, string $id)
    {
        try {
            ...
            try {
                $actor = AuditContext::actor();
                UserAuditLog::record($actor['account_id'], 'invitation_revoked', null, $actor['user_id'], $actor['user_name'], "Invitation to {$invitation->email} revoked", ['invitee_email' => $invitation->email, 'invitation_id' => $invitation->id]);
            } catch (\Throwable $e) {
                Log::warning('[AuditLog] Failed to record invitation_revoked', ['error' => $e->getMessage()]);
            }
    ...
    public function acceptInvitation(Request $request)
    {
        try {
            ...
            try {
                UserAuditLog::record($user->tenant_id, 'invitation_accepted', $user->id, $user->id, trim(($user->first_name ?? '') . ' ' . ($user->last_name ?? '')), "Invitation accepted by {$user->email}", ['invitation_id' => $invitation->id, 'invitation_age_hours' => $invitation->created_at ? round($invitation->created_at->diffInHours(now())) : null]);
            } catch (\Throwable $e) {
                Log::warning('[AuditLog] Failed to record invitation_accepted', ['error' => $e->getMessage()]);
            }
