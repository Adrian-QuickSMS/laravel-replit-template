...
<div class="modal fade" id="defaultStatusConfirmModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn" id="confirmDefaultStatusBtn">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            window.confirmDefaultStatusChange();
        });
    }
});
...
function confirmDefaultStatusChange() {
    ...
    if (!countryCode || !newStatus) {
        console.error('[CountryControls] Missing pending data:', pendingDefaultStatusChange);
        showToast('Error: No change data available', 'error');
        return;
    }
    ...
    if (!country) {
        ...
        showToast('Error: Country not found', 'error');
        return;
    }
...
overrideAccountsData[{!! json_encode($account['name']) !!}] = { id: {!! json_encode($account['id']) !!}, subAccounts: [] };
@endforeach
@endif
