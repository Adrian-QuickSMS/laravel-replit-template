...
export function validatePartial(data: Partial<RcsMessage>): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  if (data.agent) {
    errors.push(...validateAgent(data.agent, 'agent'));
  }

  if (data.content) {
    if (data.type === 'rich_card') {
      errors.push(...validateRichCard(data.content as RcsRichCard, 'content'));
    } else if (data.type === 'carousel') {
      errors.push(...validateCarousel(data.content as RcsCarousel, 'content'));
    }
  }
